...
| P1-C05 | In Progress | accepted Final Admission から immutable plan/Job/Commandを生成しtyped Domain define/read-backを実 Hostで検証。Domain Verificationをreadiness authorityへ接続し、Domain=DEFINED/Storage=BOUNDでもImage/Network=PENDINGならboot readiness=BLOCKEDを維持。Image/Network realization、Result-loss convergence、power-on/delete、public API は継続 |
...
| P1-C05 | VM create/delete with typed libvirt module | C01〜C04/B02 | Admission-derived immutable plan、typed inactive Domain define/read-back、component readiness fail-closed foundationを実装。Image/Network realization、Result-loss convergence、power-on/delete、public API を継続 | AT-CMP-001、AT-CMP-009、AT-CMP-010 |
