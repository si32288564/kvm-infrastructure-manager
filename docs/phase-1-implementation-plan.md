...
| P1-C05 | In Progress | accepted Final Admission から immutable plan/Job/Command を生成し typed Domain define/read-back を実 Host で検証。verified Image を digest-addressed cache から identity-verified Local LVM root Volume へ materialize し、target digest read-back の immutable evidence で Image=REALIZED へ進める。Network=PENDING のため boot readiness=BLOCKED を維持。Network realization、power-on/delete、public API は継続 |
...
| P1-C05 | VM create/delete with typed libvirt module | C01〜C04/B02 | Admission-derived immutable plan、typed inactive Domain define/read-back、typed Image-to-LVM materialization/read-back、component readiness fail-closed foundation を実装。Network realization、power-on/delete、public API を継続 | AT-CMP-001、AT-CMP-009、AT-CMP-010、AT-CMP-011、AT-IMG-003 |
