...
| P1-C03 | In Progress | VLAN Segment、explicit/automatic IP/MAC、Port identity、basic OVS/SR-IOV Port Binding を Final Admission に統合。Automatic IPAM、safe release/reuse、OVS/SRIOV_DIRECT pre-boot、OVS Host-side post-boot、OVN NB/SB/chassis/logical-flow/Encap/Geneve evidenceを実装。v2 intentで共有 Logical SwitchのNetwork ownershipとLSPのPort ownershipを分離し、secure endpoint・standard CLI・ownership pre-read・apply/read-backを強制するclosed production-shape runtimeを実 test KVM OVN/OVSで2 Port PASS。durable multi-worker claim/renewal、実 worker process kill、synchronous PostgreSQL の双方向 repeated failover、renewal commit 後 response-loss、512-work backlog soak、sustained endpoint latency/pool saturation からの bounded `READ_BACK_FIRST`/single apply を PASS。external IPAM、policy-timed quarantine、Router/DHCP/Security multi-object realizationは継続 |
