...
| P1-C03 | In Progress | VLAN Segment、explicit/automatic IP/MAC、Port identity、basic OVS/SR-IOV Port Binding を Final Admission に統合。Automatic IPAM、safe release/reuse、OVS/SRIOV_DIRECT pre-boot、OVS Host-side post-boot、OVN NB/SB/chassis/logical-flow/Encap/Geneve evidenceを実装。v2 intentで共有 Logical SwitchのNetwork ownershipとLSPのPort ownershipを分離し、secure endpoint・standard CLI・ownership pre-read・apply/read-backを強制するclosed production-shape runtimeを実 test KVM OVN/OVSで2 Port PASS。durable multi-worker scheduling、external IPAM、policy-timed quarantine、Router/DHCP/Security multi-object realizationは継続 |
...
| P1-C03 | VLAN/IPAM/MAC/Port Claim と basic Port Binding | A07/C02 | current Network/Subnet/Segment/Host mapping、explicit/automatic IP/MAC、release quarantine/reuse、v2 Network/LSP ownership分離、closed OVN runtime、PostgreSQL-backed multi-worker claim/read-back-first recovery、NB/SB/chassis/logical-flow/Encap/Geneve evidenceを実装。実 test KVMのproduction-shape OVN/OVSで同一 Network 2 Port reconcile、NB/SB/chassis convergence、cleanupがPASS。external IPAM、policy-timed quarantine、Router/DHCP/Security multi-object realizationを継続 | AT-NET-002〜007、AT-NET-029〜036、FI-NET-024〜026 |
