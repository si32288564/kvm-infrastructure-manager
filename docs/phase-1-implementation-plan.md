...
| P1-B03 | write-before-execute journal と typed read-back verification | A05/A06/B02 | Agent journal、closed typed backend registry、mutation 後 crash の UNKNOWN/read-back reconciliation を実装。KIM-owned state markerに加え、標準libvirt APIのtyped VM power backendと実KVM subprocess kill/read-backを検証。[typed round-trip validation](validation/p1-b03-typed-mutation-roundtrip-20260809.md)、[crash/resync validation](validation/p1-b04-runtime-crash-resync-20260809.md)、[KVM/libvirt validation](validation/p1-b10-kvm-libvirt-kill-readback-20260809.md) を保持。DB-backed full process convergenceとdisk failure hardeningを継続 | AT-EXEC-012/024、FI-AGENT-001/002/005、FI-LIBVIRT-003、FI-TRANSPORT-001〜004 |
