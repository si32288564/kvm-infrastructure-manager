...
| P1-C06 | Availability Policy/VM Binding と Workload Resilience/Recovery Queue read-only model | B06/C02 | Availability Policy/Binding/Rebind、Failure Epoch、typed Confirmation、independent Fencing/Local LVM Storage Safety Proof、typed GLOBAL/PLANNING Recovery Budget、Eligibility、explicit Recovery Operation/Plan、destination Final Admission、existing Local LVM/VM/Image/Attachment/Power materialization、UNKNOWN/read-back、pure Recovery Verification、atomic Terminal Decision/Operation VERIFIED/Epoch RECOVERED/Budget RELEASEDを実装。[Recovery Operation validation](validation/p1-recovery-operation-authority-20260811.md)、[Recovery terminal validation](validation/p1-recovery-materialization-terminal-20260811.md)、[Real two-Host qualification](validation/p1-real-two-host-kvm-recovery-20260811.md) を保持。隔離g01→g02で同一UUIDの実backend SHUTOFF→define/start/RUNNING、destination root holder、split-brain negative、完全cleanupはPASS。terminal fixtureのdefine/image/attachment Resultとpower UNKNOWN/read-backはordinary Lease/Attempt/Receipt/Verification経路へ移行済み。ただし実source boot Volume `vda`はDomainにconfiguredのままで、detach済み`vdb`をexact boot Volume Storage Proofに代用できないためoverall E2EはBLOCKED。source-root cleanup/safety authority、PCI/SR-IOV、non-empty Network、EVACUATE backendは継続 | AT-AVR-001〜023、AT-WRI-001 |
