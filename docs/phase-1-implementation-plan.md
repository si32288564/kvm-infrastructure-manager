...
| P1-C06 | Availability Policy/VM Binding と Workload Resilience/Recovery Queue read-only model | B06/C02 | Availability Policy/Binding/Rebind、Failure Epoch、typed Confirmation、independent Fencing/Local LVM Storage Safety Proof、typed GLOBAL/PLANNING Recovery Budget、Eligibility、explicit Recovery Operation/Plan、destination Final Admission、existing Local LVM/VM/Image/Attachment/Power materialization、UNKNOWN/read-back、pure Recovery Verification、atomic Terminal Decision/Operation VERIFIED/Epoch RECOVERED/Budget RELEASEDを実装。Migration 057でexact source boot `vda` observation-only Safety Proof、logical source materialization Retirement、role-aware composite Storage Safety、Eligibility/Operation/dangerous-step ABA fencingを追加。exact CP Lease-bound real helper/ordinary Result ingestionは[Lease-bound helper validation](validation/p1-real-recovery-lease-bound-helper-20260812.md)でPASS、same real Failure Epoch→Terminalは継続。[Recovery Operation validation](validation/p1-recovery-operation-authority-20260811.md)、[Recovery terminal validation](validation/p1-recovery-materialization-terminal-20260811.md)、[Real two-Host qualification](validation/p1-real-two-host-kvm-recovery-20260811.md) を保持。PCI/SR-IOV、non-empty Network、EVACUATE backend、generic source cleanupは継続 | AT-AVR-001〜025、AT-WRI-001 |
