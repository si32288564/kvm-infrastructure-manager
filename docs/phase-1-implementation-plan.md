...
| P1-B02 | Job/Command/Lease/Attempt/Result/Receipt schema and dispatcher | A02/A04 | immutable grant/result evidence、current Lease authority、Host/session generation binding、concurrent Lease exclusion、current outbound session routing、bounded Worker Lease expiry maintenance、Lease Grant と protected delivery Outbox intent の atomic commit を実装。[P1-B execution validation](validation/p1-b02-execution-authority-20260809.md)、[process runtime validation](validation/p1-b05-process-runtime-20260809.md)、[protected delivery validation](validation/p1-b06-protected-command-delivery-20260809.md) を保持。Outbox publisher/internal Bus/Gateway consumer を継続 | AT-EXEC-001〜016 |
