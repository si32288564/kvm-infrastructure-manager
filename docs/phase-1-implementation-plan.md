...
| P1-C05 | In Progress | Domain/Storage/Image、OVS/SRIOV_DIRECT pre-boot から READY、typed power-on、standard libvirt read-back、runtime power projection まで実装。実 VF hardware certification、post-boot dataplane、delete/public API は継続 |
...
| P1-C05 | VM create/delete with typed libvirt module | C01〜C04/B02 | Domain/Image/Storage/OVS/SRIOV_DIRECT Port pre-boot evidence、all-component READY transaction、typed power-on、libvirt read-back runtime projection を実装。実 VF qualification、post-boot dataplane、delete/public API を継続 | AT-CMP-001、AT-CMP-009〜013、AT-IMG-003、AT-NET-026〜027、FI-LIBVIRT-004 |
