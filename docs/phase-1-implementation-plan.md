...
| P1-C06 | Availability Policy/VM Binding と Workload Resilience/Recovery Queue read-only model | B06/C02 | Availability Policy/Binding/Rebind、Failure Epoch、typed Confirmation、independent Fencing/Local LVM Storage Safety Proof、typed GLOBAL/PLANNING Recovery Budget、Eligibility、explicit Recovery Operation/Plan、destination Final Admission、existing Local LVM/VM/Image/Attachment/Power materialization、UNKNOWN/read-back、pure Recovery Verification、atomic Terminal Decision/Operation VERIFIED/Epoch RECOVERED/Budget RELEASEDを実装。Migration 057でexact source boot `vda` observation-only Safety Proofとlogical Retirement、Migration 058でFENCED Hostのclosed read-only Verification Lease、Migration 059でgeneric Port source-quiescence/PortBindingHandoff authorityとexact OVS iface-id evidenceを追加。isolated g01→g02 Local LVM/zero-Port/no-PCI campaignはsame real PostgreSQL Failure Epoch→TerminalをPASS。non-empty Network real gateは既存OVN runtimeにtyped source unbind/retirement authorityが無くBLOCKED（Recovery shortcutは追加しない）。[Lease-bound helper validation](validation/p1-real-recovery-lease-bound-helper-20260812.md)、[Recovery Operation validation](validation/p1-recovery-operation-authority-20260811.md)、[Recovery terminal validation](validation/p1-recovery-materialization-terminal-20260811.md)、[Real two-Host qualification](validation/p1-real-two-host-kvm-recovery-20260811.md)、[non-empty Network preflight](validation/p1-real-two-host-kvm-ovn-recovery-20260812.md) を保持。PCI/SR-IOV、typed source OVN unbind/retirement、EVACUATE backend、generic source cleanupは継続 | AT-AVR-001〜026、AT-WRI-001、AT-NET-046 |
