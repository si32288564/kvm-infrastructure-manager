...
| Q-094 | In Progress | 両 candidate が basic contract と 10,000-session handoff を通過。gRPC は operational/control-path leader、typed HTTP/2 は density leader。[real gRPC authority storm](spikes/results/q094-grpc-authority-storm-20260809.md) で pre-auth protection、[TLS passthrough drain](spikes/results/q094-tls-passthrough-drain-20260809.md) で L4 proxy の mTLS identity 維持と 1,000-session hard drain/reconnect を確認。L7 proxy と durable spool/resync を継続 |
