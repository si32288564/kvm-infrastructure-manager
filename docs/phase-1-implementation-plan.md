...
| P1-C03 | In Progress | VLAN Segment、explicit IP/MAC、Port identity、basic OVS/SR-IOV Port Binding を Final Admission に統合。OVS/SRIOV_DIRECT pre-boot、OVS Host-side post-boot、immutable OVN Port intent、NB/SB/chassis、logical-flow coverage、Chassis/Encap projection、directed Geneve tunnel evidence authority を実装。実 2 物理 Host の隔離 kernel Geneve path と MTU 1400 双方向 probe は PASS。automatic IPAM、release quarantine、production OVN runtime、OVN-generated Port-bound tunnel と multi-object realization は継続 |
...
| P1-C03 | VLAN/IPAM/MAC/Port Claim と basic Port Binding | A07/C02 | current Network/Subnet/Segment/Host mapping、explicit IP/MAC Claim、Port/Binding reservation、OVN Port intent/NB/SB/chassis/logical-flow/Encap projection、directed Geneve tunnel evidence authority を実装。単一 Host の隔離 2 endpoint verifier と実 2 物理 Host の kernel Geneve/MTU qualification は PASS。automatic allocation、quarantine/release、production OVN runtime、OVN-generated current Port-bound tunnel、multi-object realization を継続 | AT-NET-002〜007、AT-NET-029〜031 |
