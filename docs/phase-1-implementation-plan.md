...
| P1-C05 | In Progress | accepted Final Admission と current root Volume Binding/Attachment Claim から immutable VM materialization plan と Job/Command を同一 transaction で生成し、closed typed libvirt inactive Domain define/read-back を実 Host で検証。Image/Network realization、Result-loss convergence、start/delete、public API は継続 |
...
| P1-C05 | VM create/delete with typed libvirt module | C01〜C04/B02 | Admission-derived immutable plan、typed inactive Domain define/read-back foundation を実装。Image/Network realization、Result-loss convergence、start/delete、public API を継続 | AT-CMP-001、AT-CMP-009 |
