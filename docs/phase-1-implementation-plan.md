...
| P1-C04 | In Progress | Local LVM Backend/Class/capacity observation、Volume、Binding Intent、single-writer Attachment Claim を Final Admission に統合。closed typed LV create、Agent kill 後の実 LVM read-back、immutable LV identity evidence、current BOUND Binding を実 Host で検証。typed libvirt attach/detach と release/fencing workflow は継続 |
...
| P1-C04 | Local LVM Volume/Attachment Claim/generation/single-writer | A07/C02 | Backend/Class/capacity observation、Volume/Binding Intent、single-writer Attachment Claim、closed typed LV create/read-back、immutable/current Binding を実装。typed libvirt attach/detach と release/fencing を継続 | AT-STO-001〜008, AT-STO-012, AT-STO-024 |
