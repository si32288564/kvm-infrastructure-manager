...
| P1-B06 | In Progress | HostGroup revision、whole-set membership、cardinality、hierarchy、Selector、External Assertion verifier/materialization、Placement fencing、Upgrade/Maintenance Snapshot consumers、Group Policy Binding、closed Placement Scope authorityを実装。Availability consumerはVM Binding、Explicit Rebind、Failure Epoch、typed Confirmation、independent typed Fencing/Local LVM Storage Safety Proof、Recovery Eligibility + transactional Planning Budget Claimまで接続。[External Assertion validation](validation/p1-b06-external-assertion-verifier-20260811.md)、[Typed safety validation](validation/p1-typed-fencing-storage-safety-20260811.md)、[Recovery Eligibility validation](validation/p1-recovery-eligibility-authority-20260811.md) を追加。population-complete EXACTLY_ONE、Recovery Operation、failure-domain maintenance schedulingは継続 |
...
| P1-C03 | In Progress | VLAN Segment、explicit/automatic IP/MAC、Port identity、basic OVS/SR-IOV Port Binding を Final Admission に統合。Automatic IPAM、safe release/reuse、OVS/SRIOV_DIRECT pre-boot、OVS Host-side post-boot、OVN NB/SB/chassis/logical-flow/Encap/Geneve evidenceを実装。v2 intentで共有 Logical SwitchのNetwork ownershipとLSPのPort ownershipを分離し、secure endpoint・standard CLI・ownership pre-read・apply/read-backを強制するclosed production-shape runtimeを実 test KVM OVN/OVSで2 Port PASS。durable multi-worker claim/renewal、graceful scale/drain、bounded Prometheus metrics、実 worker process kill、synchronous PostgreSQL の双方向 repeated failover、renewal response-loss、512-work soak、sustained endpoint latency/pool saturation からの bounded `READ_BACK_FIRST`/single apply を PASS。external IPAM、policy-timed quarantine、Router/DHCP/Security multi-object realizationは継続 |
...
| P1-C05 | In Progress | Domain/Storage/Image、OVS/SRIOV_DIRECT pre-boot から READY、typed power-on、standard libvirt read-back、runtime power projection、OVS Host-side post-boot dataplane convergence まで実装。実 VF hardware certification、OVN/E2E convergence、delete/public API は継続 |
...
| P1-B02 | Job/Command/Lease/Attempt/Result/Receipt schema and dispatcher | A02/A04 | immutable grant/result evidence、current Lease authority、Host/session generation binding、concurrent Lease exclusion、current outbound session routing、bounded Worker Lease expiry maintenance、Lease Grant と protected delivery Outbox intent の atomic commit、bounded JetStream publisher、Gateway Inbox/current-authority revalidation/Outbound Registry consumer を実装。[P1-B execution validation](validation/p1-b02-execution-authority-20260809.md)、[process runtime validation](validation/p1-b05-process-runtime-20260809.md)、[protected delivery validation](validation/p1-b06-protected-command-delivery-20260809.md)、[Worker/Gateway delivery validation](validation/p1-b07-worker-gateway-delivery-20260809.md)、[real JetStream failover validation](validation/p1-b08-real-jetstream-failover-20260809.md)、[full process fault validation](validation/p1-b09-full-process-fault-campaign-20260809.md) を保持。full-process Receipt response-loss/replay と Gateway stream write/JetStream ACK 境界の hard-kill/redelivery を実装済み。production Secret Provider rotation を継続 | AT-EXEC-001〜023 |
| P1-B03 | write-before-execute journal と typed read-back verification | A05/A06/B02 | Agent journal、closed typed backend registry、mutation 後 crash の UNKNOWN/read-back reconciliation を実装。標準 libvirt API の typed VM power backend、実 KVM subprocess kill/read-back、UNKNOWN Verification の durable Worker/Bus/Gateway delivery、remote Agent を含む full-process convergence を検証。[typed round-trip validation](validation/p1-b03-typed-mutation-roundtrip-20260809.md)、[crash/resync validation](validation/p1-b04-runtime-crash-resync-20260809.md)、[KVM/libvirt validation](validation/p1-b10-kvm-libvirt-kill-readback-20260809.md)、[Verification delivery validation](validation/p1-b11-verification-delivery-20260809.md)、[remote KVM convergence validation](validation/p1-b12-remote-kvm-full-process-20260809.md)を保持。disk-full/corrupt journal qualification を継続 | AT-EXEC-012/024〜026、FI-AGENT-001/002/005、FI-LIBVIRT-003/004、FI-TRANSPORT-001〜004 |
| P1-B04 | Host Authority generation、manual arming、reconnect non-rearm | A08/B02 | active Command Lease を Host authority/session generation へ bind し、Gateway current-session registry、long-running Agent Session Runner、read-only resync dispatch gate、kim-host-agent executable lifecycle へ接続。durable accepted-generation ledger と bounded reconnect を実装。[process runtime validation](validation/p1-b05-process-runtime-20260809.md) を保持 | AT-EXEC-013/014、FI-AGENT-006、FI-GATEWAY-002/003、FI-HLC-008/013、FI-TRANSPORT-003/004 |
...
| P1-B06 | explicit HostGroup/Placement Pool membership generation | A02/B05 | first-class revision/current authority、whole-set generation、atomic current switch、set-backed snapshot、SYSTEM scope cardinality、TREE hierarchy、typed Selector、purpose-limited Ed25519 External Assertion verifier/materialization、Placement fencing、Upgrade/Maintenance Snapshot consumers、cross-domain disruption fencing、MAINTENANCE Group Policy resolution/Plan provenance、closed VM_PLACEMENT Scope authorityを実装。population authority、Availability policy consumers、failure-domain maintenance schedulingを継続 | AT-HGR-001/002/003/004/005/007/010/016/017/018/019/020/021/022/023/024/025、FI-HGR-002/003/004/005/009/010/011/012/013/014/015/016/017/018 |
...
| P1-C03 | VLAN/IPAM/MAC/Port Claim と basic Port Binding | A07/C02 | current Network/Subnet/Segment/Host mapping、explicit/automatic IP/MAC、release quarantine/reuse、v2 Network/LSP ownership分離、closed OVN runtime、PostgreSQL-backed multi-worker claim/renewal/read-back-first recovery、NB/SB/chassis/logical-flow/Encap/Geneve evidenceを実装。実 test KVMのproduction-shape OVN/OVSで同一 Network 2 Port reconcile、NB/SB/chassis convergence、cleanupがPASS。実 worker process kill、synchronous PostgreSQL 双方向 repeated failover、renewal response-loss、512-work soak、sustained latency/pool saturation 後も generation 2 read-back で duplicate apply なく収束。external IPAM、policy-timed quarantine、Router/DHCP/Security multi-object realizationを継続 | AT-NET-002〜007、AT-NET-029〜043、FI-NET-024〜029、FI-DB-003〜006 |
...
| P1-C05 | VM create/delete with typed libvirt module | C01〜C04/B02 | Domain/Image/Storage/OVS/SRIOV_DIRECT Port pre-boot evidence、all-component READY transaction、typed power-on、libvirt read-back runtime projection、OVS Host-side post-boot dataplane projection を実装。実 VF qualification、OVN/E2E convergence、delete/public API を継続 | AT-CMP-001、AT-CMP-009〜013、AT-IMG-003、AT-NET-026〜028、FI-LIBVIRT-004、FI-NET-020 |
| P1-C06 | Availability Policy/VM Binding と Workload Resilience/Recovery Queue read-only model | B06/C02 | Availability Policy/Binding/Rebind、Failure Epoch、typed Confirmation、independent Fencing/Local LVM Storage Safety Proof、typed GLOBAL/PLANNING Recovery Budget、Eligibility、explicit Recovery Operation/Plan、destination Final Admission、existing Local LVM/VM/Image/Attachment/Power materialization、UNKNOWN/read-back、pure Recovery Verification、atomic Terminal Decision/Operation VERIFIED/Epoch RECOVERED/Budget RELEASEDを実装。Migration 057でexact source boot `vda` observation-only Safety Proofとlogical Retirement、Migration 058でFENCED/ARMED Hostのclosed read-only Verification Lease、Migration 059でgeneric Port source-quiescence/PortBindingHandoff authorityとexact OVS iface-id evidence、Migration 060でgeneric OVN Port UNBIND/retirement authority、Migration 061でRecovery Port/Binding incarnation history、Migration 062でrepeated OVN retirement/Handoff exact-incarnation authorityを追加。g01→g02 Local LVM/zero-Portおよびnon-empty OVS Port/no-PCI campaignはsame real PostgreSQL Failure Epoch→TerminalをPASSし、PostgreSQL 17で同一Port `1/1→2/2→3/3` repeated authority chainをPASS。[Lease-bound helper validation](validation/p1-real-recovery-lease-bound-helper-20260812.md)、[Recovery Operation validation](validation/p1-recovery-operation-authority-20260811.md)、[Recovery terminal validation](validation/p1-recovery-materialization-terminal-20260811.md)、[Real two-Host qualification](validation/p1-real-two-host-kvm-recovery-20260811.md)、[non-empty Network qualification](validation/p1-real-two-host-kvm-ovn-recovery-20260812.md)、[repeated retirement validation](validation/p1-c03-repeated-ovn-port-retirement-20260813.md)を保持。real repeated two-Host Recovery、PCI/SR-IOV、EVACUATE backend、generic source/backend cleanup、soak/chaosは継続 | AT-AVR-001〜026、AT-WRI-001、AT-NET-046〜048 |
...
| P1-D03 | Release Manifest、artifact digest、schema readiness、single-target canary | A01/A02 | In Progress: verified `.deb`、real dpkg lock contention、Result loss を single install へ収束。実 `postinst` interruption を quarantine 後、通常 Attempt と分離した immutable `CONFIGURE_EXISTING` Recovery Plan / Attempt / Verification / explicit rearm へ収束。Campaign は auto-resume しない。実 KIM component、Wave budget、campaign abort / rollback は継続 | AT-UPG-002〜007、AT-UPG-029〜037、FI-UPG-019〜027 |
| P1-D04 | Debian/Ubuntu package、systemd hardening、containerized Control Plane、offline install | A01/A05 | In Progress: isolated Debian/systemd Target backend、package database contention、incomplete package quarantine、closed package recovery の実 Host qualification が PASS。v1 package pipeline の KIM artifact、offline bundle、product unit への移植は継続 | AT-UPG-034〜037、AT-OFFLINE-001、AT-DEPLOY-001/002 |
| P1-D05 | second Linux family adapter/package/validation lane | A06/A07/D04 | new adapter/package | AT-AGT-006/008 |
| P1-D06 | metrics、audit、diagnostic evidence、operator runbook | all | OVN worker lifecycle/execution/renewal/pool/backlog Prometheus metrics を実装。v1 metrics/audit reuse と product-wide alert/runbook を継続 | AT-AUD-001/002、AT-O11Y-001/002 |
