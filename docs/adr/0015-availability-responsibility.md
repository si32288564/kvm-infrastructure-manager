...
- `fencing_requirements` / `storage_requirements` text slotもruntime解釈しない。exact AvailabilityPolicy revisionからindependent typed FencingPolicy / StorageSafetyPolicy revisionを参照し、pure Evaluationとpositive Proofを分離する。Fencing Proofだけがproof-cause付き`CONFIRMED → FENCED`を許し、Storage Safety Proofは別authorityのまま保持する。heartbeat/Agent loss、UNKNOWN、片方または両proofをRecovery Eligibility/Operationへ昇格させない。
