...
Public delete is narrower than create. Migrations 087/089/090 and their composite qualification permit zero or one STANDARD Port with one ROOT Volume and at most one DATA Volume. Migration 091 additionally permits two STANDARD Ports with one ROOT Volume. All require no PCI, delete protection disabled, and exact observed `SHUTOFF`. Terraform first requests typed SHUTOFF convergence and then requests verified deletion. Port deletion preserves logical Port/MAC/IP identity. ROOT+DATA delete preserves both logical Volumes, capacity claims and verified backend materializations. The maximum two-Port-plus-DATA create profile remains fail-closed for delete.
