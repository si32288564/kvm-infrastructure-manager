...
Public delete is narrower than create. Migrations 087/089/090 and their composite qualification permit zero or one STANDARD Port with one ROOT Volume and at most one DATA Volume; all require no PCI, delete protection disabled, and exact observed `SHUTOFF`. Terraform first requests typed SHUTOFF convergence and then requests verified deletion. One-Port delete preserves logical Port/MAC/IP identity. ROOT+DATA delete requires exact absence for both attachments and preserves both logical Volumes, capacity claims and verified backend materializations. Two-Port delete remains fail-closed.
