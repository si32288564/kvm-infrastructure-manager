...
Public delete is narrower than create. Migrations 087/089 qualify zero Port or one STANDARD Port, exactly one ROOT Volume, no PCI, delete protection disabled, and exact observed `SHUTOFF`. Terraform first requests typed SHUTOFF convergence and then requests verified deletion. A one-Port delete consumes exact OVN/OVS retirement evidence and preserves the logical Port/MAC/IP as unattached. Two-Port or DATA-Volume delete remains fail-closed.
