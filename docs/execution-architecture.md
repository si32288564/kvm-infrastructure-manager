...
Recovery Operationも既存Execution Planeを迂回しない。Migration 054のclosed preparation stepに加え、Migration 055のdestination define/image/attachment/powerは既存typed Job/Command/Lease/Attempt/Verificationを再利用する。Recovery codeはlibvirt Start、LVM、network backendを直接呼ばない。power `UNKNOWN`はblind retryせずstandard libvirt read-backを先行し、MATCHEDでもexact Recovery Verificationまでは`VERIFYING`に留まる。Command successまたはJob successをRecovery terminal causeにしない。
