...
Recovery Operationも既存Execution Planeを迂回しない。Migration 054はdestination Final Admission後にclosed `HOST_AGENT_STATE_MARKER_ENSURE / kim.command.host-agent-state-marker/v1` preparation stepを通常Job/Command authorityとして発行し、Lease/Attempt/Result/Verification semanticsを再利用する。このmarkerはKIM-owned execution path qualificationでありlibvirt VM recoveryではない。Command `UNKNOWN`はblind retryせずread-backし、MATCHEDでもOperationは`VERIFYING`に留まる。
