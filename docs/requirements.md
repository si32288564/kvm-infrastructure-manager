...
| IAC-019 | Terraform ProviderはNorthbound APIだけを使用し、Bearer automation principal、Problem code/request ID、ETag/If-Match、cross-process Create Idempotency-Key、authorized refresh、contract import、bounded Operation pollingを実装する。PostgreSQL、Agent、backendへ直接接続しない | Must |
| IAC-020 | Northbound clientはclient identityとclient-owned logical resource referenceからprocess再起動後も同一Create identityを再構成でき、KIMはそのidentityをexact desired digestへbindする。display name検索によるadoption、異client/referenceのadoption、同referenceの異intentは許可しない | Must |
