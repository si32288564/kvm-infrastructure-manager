...
Migration 054はEligibilityとExecutionの間にexplicit `RecoveryOperationRequest`、one immutable `RecoveryOperation/RecoveryPlan`、append-only transition/current projectionを追加する。Request/Planはexact Decision/RESERVED Budget Claim/historical Binding/Policy/actionとEligibility candidateをbindするだけでside effectを持たない。Phase 1 executable actionは`RESTART_ON_OTHER_HOST`のdestination admission/preparation foundationに限定し、`EVACUATE` backendはfail closedの後続gateとする。
