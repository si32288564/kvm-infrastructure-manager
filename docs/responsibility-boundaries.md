...
Migration 055ではRecovery Materializationをdestination desired-state orchestration provenance、Recovery Verificationをexact multi-domain read-back authority、Recovery Terminal Decisionをincident completion authorityとしてさらに分離する。Verification `VERIFIED`だけではOperation/Epoch/Budgetを変更せず、Terminal Decision transactionだけが三つを整合的に進める。Budget releaseはrecovery success authorityであってsource cleanupまたはresource garbage collection authorityではない。
