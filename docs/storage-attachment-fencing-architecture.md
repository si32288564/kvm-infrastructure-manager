...
Local LVM is one typed materialization implementation and remains planned-mobility-only. Host, VG/LV UUID, path, binding/materialization/copy/cleanup generations are computed physical incarnations and cannot be Terraform desired drift. Migrations 068–072 remain the source of truth for content-preserving relocation and exact source reclamation. Migration 081 now exposes backend-neutral `/api/v1/volumes` and Terraform `kim_volume` without those incarnations; public Attachment, Ceph RBD, resize, and VM Phase 3 remain unimplemented. See [ADR-0034](adr/0034-independent-volume-capacity-materialization-and-attachment-authority.md), [ADR-0035](adr/0035-phase2-northbound-logical-resource-contract.md), and the [Phase 2 validation](validation/p2-northbound-terraform-resource-contract-20260815.md).
...
Migration 055ではsource Storage Safety Proofを「sourceから安全にrecover awayできるauthority」、destination Attachment observationを「destination LVがVMへ正しく接続されたauthority」として分離する。destinationは既存Local LVM Binding/Attachment Commandとdevice/holder read-backを使用する。power直前とterminal transactionでsource proof usabilityおよびdestination Attachment generationを再検証し、`ATTACHED→DETACHED→ATTACHED`のold evidenceを再利用しない。Recovery failureからVolume detach/deleteまたはClaim releaseを推測しない。

Migration 057はsource boot rootにclosed `LOCAL_LVM_SOURCE_ROOT_QUIESCED_NO_HOLDER` authorityを追加する。root identityはFailure Epoch→source Admission→immutable materialization planから導出し、exact `vda`/Volume/Binding/LV/Attachment、actual libvirt `SHUTOFF/MATCHED`、configured device/source identity、holder closedを要求する。inactive Domainにconfigured `vda`が残ることはactive QEMU holderと同義ではない。専用Agent backendはread-backだけを行い、root attach/detach/delete/undefine/cleanupを持たない。secondary `SOURCE_DETACHED_NO_HOLDER`の意味は変更しない。composite source Storage Safetyはroot positive Proofと全data attachmentのexisting positive evidenceを別provenanceで要求する。

`SourceMaterializationRetirementDecision`はexact Fencing ProofとRoot Safety Proofにbindし、一つのsource incarnationをlogical `RETIRED`にする。これはsource Domain/LV/Volume/evidenceの削除、root detach、artifact cleanup、compute fencingではない。
