...
一度に取得した claim は process 内の serial queue に保持せず、`BatchLimit` を同時実行上限として各 item の authority check と renewal loop を直ちに開始します。これにより batch 前半の long-running operation が後半 item の claim lifetime を処理開始前に消費することを防ぎます。item-local adapter error は error handler へ報告し、long-lived worker は bounded poll を継続します。一方、DB claim/renewal authority error は process を停止し、old-primary path や authority loss を同一 process 内で推測 retry しません。`BatchLimit` は throughput tuning だけでなく process-local admission limit です。全 worker replica の `BatchLimit` 合計は PostgreSQL pool、OVN endpoint、Host/backend capacity より小さい certified profile とし、capacity 超過を retry/expiry storm で吸収しません。

long-running adapter operation は current claim と同じ owner/generation のまま、bounded interval で renewal を要求できます。renewal transaction は DB authority time で未失効であることを再検証し、claim 開始時に固定した maximum expiry を超えない new expiry と単調増加する renewal generation を immutable evidence へ commit します。expired claim、foreign owner、old generation を時刻更新で復活させません。

renewal failure または response loss を観測した worker は adapter operation を cancel しますが、renewal transaction の rollback、side effect 不在、claim expiry を推測しません。commit 後に response だけが失われた場合、PostgreSQL の renewed expiry は引き続き current authority であり、その期限前に successor claim を発行しません。expiry 後は旧 attempt を `DISPATCH_UNKNOWN` として保持し、新 claim generation を `READ_BACK_FIRST` に限定します。maximum lifetime を超えた local continuation、旧 worker の completion、blind duplicate apply を許可しません。
...
Migration 060はそのgeneric lifecycle operationを`UNBIND`として追加する。対象はexact Port/Binding/source Host incarnationであり、logical Port、IP Claim、MAC Claimを削除しない。typed adapterはstable KIM ownership markerをpre-readし、requested chassisだけを解除する。positive terminal authorityはNB上のlogical identity/ownership維持、SB上のsource chassis inactivity、source Host OVS上の同一`iface-id` absenceのimmutable read-back集合である。apply response lossは同じturnの成功推測に使わず`DISPATCH_UNKNOWN`へ記録し、successor claimの`READ_BACK_FIRST`だけがterminal evidenceを発行できる。同じbinding generationのordinary intent revivalは旧retirement projectionを`STALE`にし、Recovery source quiescence/Handoffから除外する。

Migration 062はretirement projectionを`(port_id, port_generation, binding_generation)`で識別し、同じlogical Portの複数Recovery incarnationを独立保持する。Port単位のlatest projectionは説明用のrebuildable pointerであり、source quiescenceやHandoffは必ずexact incarnationをjoinする。既にcommit済みのhistorical operationはcurrent Portが後続generationへ進んだ後でも同じoperation/work/evidenceへresponse-loss replayできるが、別operation identityまたは旧quiescenceを新generationへupliftできない。`PortBindingHandoff` current projectionもstrictly-later destination generationだけに置換され、immutable Handoff historyとPort/MAC/IP identityは保持される。
