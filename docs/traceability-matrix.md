...
| AVR-021 | availability-responsibility-architecture, failure-model, recovery-storm-control-architecture, placement-architecture, execution-architecture, storage-attachment-fencing-architecture, network-resource-architecture | ADR-0006, ADR-0007, ADR-0010, ADR-0015, ADR-0017, ADR-0019, ADR-0020 | INV-AVR-034, INV-AVR-035, INV-AVR-036 | AT-AVR-023, FI-AVR-017 | Implemented for closed `RESTART_ON_OTHER_HOST` no-PCI/zero-Port authority regression (migration 055/056: exact destination materialization、pre-power root identity gate、typed power UNKNOWN/read-back、post-power root holder、pure multi-domain Verification、atomic Terminal Decision)。terminal fixtureのdefine/image/attachment Resultとpower UNKNOWN/RESYNC observationはordinary execution persistenceへ移行済み。隔離g01→g02の同一UUID physical backend sequenceはPASSだが、source boot `vda`はconfiguredのままでdetached `vdb`とexact identityが異なるため、current Storage Safety Policyによるreal terminal E2EはBLOCKED。source-root cleanup/safety authority、PCI/SR-IOV、non-empty Network、EVACUATE backendは継続 |
