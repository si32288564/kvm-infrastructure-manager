...
Migration 073–076 の Project、Flavor、SYSTEM Availability Policy、ImageはPhase 1 persistent resource referenceです。Image metadata commitは同期201、artifact ingestionは202 Operationです。callerはexpected digestとapproved source IDだけを指定し、observed digestはtyped Agent read-back evidenceからのみ導出します。Operation `SUCCEEDED`はimmutable verification terminalと同義で、Command successだけでは成立しません。実査結果は [KIM Northbound API / Terraform Readiness Review](reviews/kim-terraform-api-readiness-review-20260814.md) を参照します。

Migration 081はMigration 077–080のNetwork、Subnet、unattached Port、backend-neutral Volumeを同じpublic lifecycleへ接続します。Createの認証主体/idempotency/digest/revision bindingだけを追加し、segment、MAC/IP allocation、Host、backend、VG/LV、binding、materialization generationをpublic desiredへ昇格しません。Create/Update/Deleteが返すOperationは既存typed workerとread-back terminalのprojectionであり、Providerがbackend convergenceを推測しません。詳細は [ADR-0035](adr/0035-phase2-northbound-logical-resource-contract.md) に従います。

Phase 3 VMは`virtual_machines_current`を直接公開しません。logical VM revision、runtime intent generation、exact dependency snapshot、aggregate Operation/verification/terminalを追加し、existing Final Admission、materialization readiness、typed power read-back、Recovery、EVACUATEをexact consumerとして束ねます。physical incarnation変更はdesired driftではありません。proposed contractは [VM Aggregate Resource Architecture](vm-aggregate-resource-architecture.md) と [ADR-0036](adr/0036-vm-logical-aggregate-over-physical-incarnations.md) に記録します。

Northbound Create recoveryではclientがstable client identity/resource referenceからIdempotency-Keyを再構成し、KIMがauthenticated principal/path scopeとcanonical desired digestへbindします。KIM stable resource IDが最終authorityです。client reference、Terraform address、display nameはresource authorityではなく、同じkeyの異desired digestはconflictです。process/state-write lossをname lookup adoption、delete/recreate、backend discoveryで処理しません。
