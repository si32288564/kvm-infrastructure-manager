...
Migration 073/074 の Project+Flavor は、同期的な PostgreSQL-only mutation、OIDC/RBAC、revision、idempotency、audit、OpenAPI を複数resourceで成立させたreference implementationです。共通transport/security contractとresource固有authorityを分離し、Flavor revision更新は既存VMをretrofitしません。Image、Availability Policy、backend-convergent Operation、Terraform Provider/UIは未実装またはBLOCKEDです。実査結果は [KIM Northbound API / Terraform Readiness Review](reviews/kim-terraform-api-readiness-review-20260814.md) を参照します。
