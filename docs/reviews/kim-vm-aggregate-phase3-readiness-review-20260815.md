...
```text
PHASE3_VM_AGGREGATE_DESIGN = READY FOR REVIEW
PHASE3_VM_IMPLEMENTATION   = NOT STARTED
NORTHBOUND_VM_RESOURCE     = BLOCKED
TERRAFORM_VM_RESOURCE      = BLOCKED
```

Implementation should not start by serializing `virtual_machines_current`. The repository has strong child authorities but lacks the logical aggregate producer and terminal that make them a safe public resource.
