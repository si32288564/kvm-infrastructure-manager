...
| INV-NET-032 | OVN runtime worker は PostgreSQL の current work claim owner/generation/expiry を持たずに apply または observation acceptance を行わない。expired/uncertain claim の再取得は `READ_BACK_FIRST` とし、旧 worker の遅延結果を current authority へ進めない | AT-NET-036, FI-NET-026 |
