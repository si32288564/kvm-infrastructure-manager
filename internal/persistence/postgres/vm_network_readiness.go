...
func AcceptOVSPortRealizationAndMaybeArmPower(ctx context.Context, db TxBeginner, v OVSPortRealizationObservation) error {
	...
	return pgx.BeginTxFunc(ctx, db, pgx.TxOptions{IsoLevel: pgx.ReadCommitted}, func(tx pgx.Tx) error {
		...
		if _, err := tx.Exec(ctx, `INSERT INTO kim.vm_network_port_realizations_current(vm_id,vm_generation,port_id,port_generation,binding_generation,observation_generation,evidence_id,preboot_state) VALUES($1,$2,$3,$4,$5,$6,$7,'REALIZED') ON CONFLICT(vm_id,port_id) DO UPDATE SET vm_generation=EXCLUDED.vm_generation,port_generation=EXCLUDED.port_generation,binding_generation=EXCLUDED.binding_generation,observation_generation=EXCLUDED.observation_generation,evidence_id=EXCLUDED.evidence_id,preboot_state='REALIZED',updated_at=statement_timestamp() WHERE (kim.vm_network_port_realizations_current.vm_generation,kim.vm_network_port_realizations_current.port_generation,kim.vm_network_port_realizations_current.binding_generation,kim.vm_network_port_realizations_current.observation_generation)<(EXCLUDED.vm_generation,EXCLUDED.port_generation,EXCLUDED.binding_generation,EXCLUDED.observation_generation)`, v.VMID, v.VMGeneration, v.PortID, v.PortGeneration, v.BindingGeneration, v.ObservationGeneration, v.EvidenceID); err != nil {
			return err
		}
