	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{IsoLevel: pgx.ReadCommitted}, func(tx pgx.Tx) error {
		...
		if errors.Is(err, pgx.ErrNoRows) {
			...
			if err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1
				...
				WHERE r.relocation_authority_id=$1 AND r.vm_id=$2 AND r.vm_generation=$3 AND r.source_admission_id=$4 AND r.source_host_id=$5 AND r.destination_admission_id=$6 AND r.destination_host_id=$7 AND r.destination_materialization_generation=$8
				AND (r.volume_count=1 OR r.volume_count=(SELECT count(*) FROM kim.vm_materialization_relocation_volume_evidence member
					...
					JOIN kim.volume_backend_bindings_current member_binding ON member_binding.binding_id=member.destination_binding_id AND member_binding.binding_generation=member.destination_binding_generation AND member_binding.volume_id=member.destination_volume_id AND member_binding.lv_uuid=member.destination_lv_uuid AND member_binding.host_id=r.destination_host_id AND member_binding.binding_state='BOUND'
					JOIN kim.volumes_current member_volume ON member_volume.volume_id=member.destination_volume_id AND member_volume.placement_admission_id=r.destination_admission_id
					WHERE member.relocation_authority_id=r.relocation_authority_id)))`, request.RelocationAuthorityID, request.VMID, vmGeneration, existingAdmission, existingHost, request.AdmissionID, hostID, request.MaterializationGeneration).Scan(&authorized); err != nil || !authorized {
