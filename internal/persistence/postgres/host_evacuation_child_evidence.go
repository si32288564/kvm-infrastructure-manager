...
func EvaluateHostEvacuationChildEvidence(ctx context.Context, db TxBeginner, claim HostEvacuationClaim, verificationID, destinationBindingID, destinationAdmissionID string) (HostEvacuationChildVerification, error) {
	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{}, func(tx pgx.Tx) error {
		...
		if phase != "SOURCE_QUIESCED" || currentAuthority != sourceAuthority || childPlanGeneration == 0 || storageCount != 0 || networkCount != 0 || pciCount != 0 || destinationStorageCount != 0 || destinationNetworkCount != 0 || destinationPCICount != 0 {
			return ErrHostEvacuationBlocked
		}
		...
		if _, err := tx.Exec(ctx, `INSERT INTO kim.host_evacuation_child_verification_evidence(verification_id,child_operation_id,child_generation,child_plan_generation,vm_id,vm_generation,source_host_id,destination_host_id,destination_admission_id,quiescence_evidence_id,destination_binding_id,source_storage_state,source_network_state,source_pci_state,destination_power_state,destination_storage_state,destination_network_state,destination_pci_state,source_ownership_state,source_host_authority_generation,destination_materialization_generation,destination_power_observation_generation,verification_state,verification_digest) VALUES($1,$2,1,$3,$4::uuid,$5,$6,$7,$8,$9,$10,'NOT_REQUIRED','NOT_REQUIRED','NOT_REQUIRED','RUNNING','NOT_REQUIRED','NOT_REQUIRED','NOT_REQUIRED','RETIRED',$11,$12,$13,'VERIFIED',$14)`, verificationID, claim.ChildOperationID, childPlanGeneration, vmID, vmGeneration, sourceHost, destinationHost, destinationAdmissionID, quiescenceID, destinationBindingID, sourceAuthority, readyGeneration, powerGeneration, verificationDigest); err != nil {
			return err
		}
