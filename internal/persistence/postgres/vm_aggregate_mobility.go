...
func AssociateVMAggregateMobility(ctx context.Context, db TxBeginner, r VMAggregateMobilityAssociationRequest) (VMAggregateMobilityAssociation, error) {
	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{IsoLevel: pgx.Serializable}, func(tx pgx.Tx) error {
		...
		if err := tx.QueryRow(ctx, `SELECT port_count,volume_count FROM kim.vm_dependency_snapshot_evidence WHERE dependency_snapshot_id=$1`, dependencyID).Scan(&portCount, &volumeCount); err != nil || portCount < 0 || portCount > maxVMAggregatePorts || volumeCount < 1 || volumeCount > maxVMAggregateVolumes {
			return ErrVMAggregateConflict
		}
		portSet := ""
		if portCount == 0 {
		...
		if r.MobilityKind == "RECOVERY" {
			...
			if err := tx.QueryRow(ctx, `SELECT d.desired_digest||':'||r.local_lvm_copy_terminal_digest
				FROM kim.vm_dependency_volume_evidence d
				JOIN kim.vm_materialization_relocation_authority_evidence r ON r.vm_id=$2 AND r.source_admission_id=$3 AND r.source_host_id=$4 AND r.destination_admission_id=$5 AND r.destination_host_id=$6 AND r.volume_count=1
				WHERE d.dependency_snapshot_id=$1 AND d.volume_ordinal=0 AND d.device_role='ROOT'`, dependencyID, r.VMID, sourceAdmission, sourceHost, terminal.destinationAdmission, terminal.destinationHost).Scan(&volumeSet); err != nil {
				return ErrVMAggregateConflict
			}
		} else {
			var matched int
			if err := tx.QueryRow(ctx, `SELECT count(*),coalesce(string_agg(d.volume_ordinal||':'||d.desired_digest||':'||member.member_digest,',' ORDER BY d.volume_ordinal),'')
				FROM kim.vm_dependency_volume_evidence d
				JOIN kim.vm_materialization_relocation_authority_evidence r ON r.vm_id=$2 AND r.source_admission_id=$3 AND r.source_host_id=$4 AND r.destination_admission_id=$5 AND r.destination_host_id=$6 AND r.volume_count=$7
				JOIN kim.vm_materialization_relocation_volume_evidence member ON member.relocation_authority_id=r.relocation_authority_id AND member.volume_ordinal=d.volume_ordinal AND member.device_role=d.device_role AND member.source_volume_id=d.volume_id
				...
				JOIN kim.volume_backend_bindings_current binding ON binding.binding_id=member.destination_binding_id AND binding.binding_generation=member.destination_binding_generation AND binding.volume_id=member.destination_volume_id AND binding.lv_uuid=member.destination_lv_uuid AND binding.host_id=$6 AND binding.binding_state='BOUND'
				JOIN kim.volumes_current volume ON volume.volume_id=member.destination_volume_id AND volume.placement_admission_id=$5
				WHERE d.dependency_snapshot_id=$1`, dependencyID, r.VMID, sourceAdmission, sourceHost, terminal.destinationAdmission, terminal.destinationHost, volumeCount).Scan(&matched, &volumeSet); err != nil || matched != volumeCount {
				return ErrVMAggregateConflict
			}
		}
