...
func BuildHostEvacuationDestinationPlacementRequest(ctx context.Context, db TxBeginner, claim HostEvacuationClaim, requestID, destinationHostID string) (PlacementAdmissionRequest, error) {
	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{AccessMode: pgx.ReadOnly}, func(tx pgx.Tx) error {
		...
		var bootable bool
		if err := tx.QueryRow(ctx, `SELECT a.project_id,a.workload_id,a.image_id,a.flavor_id,a.pool_id,e.source_host_id,v.storage_class_id,v.storage_class_revision,v.size_bytes,v.access_mode,v.bootable,ce.fencing_policy_revision,b.backend_id,b.vg_uuid,b.backend_generation,p.capacity_generation
			...
			WHERE c.child_operation_id=$1 AND c.phase='SOURCE_QUIESCED' AND b.host_id<>e.source_host_id`, claim.ChildOperationID, destinationHostID).Scan(&out.ProjectID, &out.WorkloadID, &out.ImageID, &out.FlavorID, &out.PoolID, &sourceHost, &classID, &classRevision, &sizeBytes, &accessMode, &bootable, &fencingRevision, &backendID, &vgUUID, &backendGeneration, &capacityGeneration); err != nil {
			return ErrHostEvacuationBlocked
		}
