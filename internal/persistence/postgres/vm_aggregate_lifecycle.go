...
func CompleteVMAggregateDelete(ctx context.Context, db TxBeginner, claim VMAggregateClaim, domainAbsenceID, attachmentEvidenceID, storageAbsenceID, computeReleaseID, terminalID, tombstoneID string) (string, error) {
	...
	var replayDomain, replayStorage, replayRelease string
	if err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{AccessMode: pgx.ReadOnly}, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx, `SELECT domain_absence_evidence_id,storage_absence_evidence_id,compute_release_evidence_id FROM kim.vm_delete_terminal_evidence WHERE terminal_evidence_id=$1 AND delete_operation_id=$2 AND operation_generation=$3`, terminalID, claim.OperationID, claim.OperationGeneration).Scan(&replayDomain, &replayStorage, &replayRelease)
	}); err == nil {
		if replayDomain != domainAbsenceID || replayStorage != storageAbsenceID || replayRelease != computeReleaseID {
			return "", ErrVMAggregateConflict
		}
		return terminalID, nil
	} else if !errors.Is(err, pgx.ErrNoRows) {
	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{}, func(tx pgx.Tx) error {
		...
		terminalDigest := digestVMAggregate(map[string]any{"terminal_id": terminalID, "operation_id": claim.OperationID, "domain_absence_id": domainAbsenceID, "storage_absence_id": storageAbsenceID, "compute_release_id": computeReleaseID, "state": "VERIFIED"})
		if _, err := tx.Exec(ctx, `INSERT INTO kim.vm_delete_terminal_evidence(terminal_evidence_id,delete_operation_id,operation_generation,domain_absence_evidence_id,storage_absence_evidence_id,compute_release_evidence_id,terminal_state,terminal_digest) VALUES($1,$2,$3,$4,$5,$6,'VERIFIED',$7)`, terminalID, claim.OperationID, claim.OperationGeneration, domainAbsenceID, storageAbsenceID, computeReleaseID, terminalDigest); err != nil {
			return err
		}
