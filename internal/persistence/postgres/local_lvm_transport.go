...
func PrepareLocalLVMTransportSession(ctx context.Context, db TxBeginner, request LocalLVMTransportSessionRequest) (LocalLVMTransportSession, error) {
	...
	err := pgx.BeginTxFunc(ctx, db, pgx.TxOptions{}, func(tx pgx.Tx) error {
		...
		if err := tx.QueryRow(ctx, `SELECT copy.copy_generation,copy.source_host_id,copy.source_volume_id,copy.source_binding_id,copy.source_binding_generation,copy.source_vg_uuid,copy.source_lv_uuid,
			...
			LEFT JOIN kim.host_evacuation_source_storage_volume_safety_evidence safety_member ON safety_member.safety_member_evidence_id=copy.source_volume_safety_evidence_id AND safety_member.safety_member_digest=copy.source_volume_safety_digest AND safety_member.child_operation_id=copy.child_operation_id AND safety_member.child_generation=copy.child_generation AND safety_member.volume_id=copy.source_volume_id AND safety_member.binding_id=copy.source_binding_id AND safety_member.binding_generation=copy.source_binding_generation AND safety_member.safety_state='SAFE'
			...
			JOIN kim.volume_attachment_observations_current source_holder ON source_holder.attachment_id=COALESCE(safety_member.attachment_id,safety.root_attachment_id) AND source_holder.attachment_generation=COALESCE(safety_member.attachment_generation,safety.root_attachment_generation) AND source_holder.binding_id=copy.source_binding_id AND source_holder.binding_generation=copy.source_binding_generation AND source_holder.host_id=copy.source_host_id AND source_holder.observed_lv_uuid=copy.source_lv_uuid AND NOT source_holder.holder_open
