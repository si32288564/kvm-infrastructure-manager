...
func TestDataplaneRequiresActiveOVSIdentity(t *testing.T) {
	...
	backend := DataplaneBackend{Client: fakeDataplaneClient{DataplaneObservation{DomainRunning: true, InterfacePresent: true, BridgeMatches: true, TargetDevice: "vnet7", Bridge: "br-int", LinkState: "up"}}}
	result, err := backend.Execute(context.Background(), contract.CommandLease{TargetResourceID: "port:port-1", CommandPayload: payload, AttemptIndex: 1})
	if err != nil || result.Outcome != "SUCCEEDED" {
		t.Fatalf("result=%#v err=%v", result, err)
	}
	degraded := DataplaneBackend{Client: fakeDataplaneClient{DataplaneObservation{DomainRunning: true, InterfacePresent: true, BridgeMatches: true, TargetDevice: "vnet7", Bridge: "br-int", LinkState: "down"}}}
