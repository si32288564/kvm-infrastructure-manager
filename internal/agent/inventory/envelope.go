...
func NewEnvelope(snapshot Snapshot, sessionGeneration uint64, messageID string) (session.Envelope, error) {
	...
	envelope := session.NewEnvelope(snapshot.HostIdentity, sessionGeneration, session.StreamInventory, messageID, SnapshotSchemaV3, "host-inventory", snapshot.ObservationGeneration, payload)
