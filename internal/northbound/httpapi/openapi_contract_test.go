...
func TestOpenAPIProjectContractAndLifecycleMetadata(t *testing.T) {
	...
	for _, name := range []string{"Network", "Subnet", "Port", "Volume"} {
	...
	publicDesired, _ := json.Marshal([]any{schemas["ProjectCreate"], schemas["FlavorDesired"], schemas["AvailabilityPolicyDesired"], schemas["ImageDesired"], schemas["NetworkDesired"], schemas["SubnetDesired"], schemas["PortDesired"], schemas["VolumeDesired"]})
